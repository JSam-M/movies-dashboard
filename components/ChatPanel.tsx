...
export default function ChatPanel({ movies, onClose, initialMessage }: Props) {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const hResult = heuristicRecommend(content, movies)
      const needsApi = await fetch('/api/chat', {
        method: 'POST',
      ...
      const fallback = heuristicRecommend(content, movies)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end p-4 sm:p-6 pointer-events-none">
      ...
      <div
        className="relative pointer-events-auto flex flex-col rounded-3xl overflow-hidden w-full sm:w-[420px]"
        style={{
          height: '85vh',
          maxHeight: '680px',
                ...
                <div className="flex gap-1.5 items-center h-4">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="w-1.5 h-1.5 rounded-full"
                      style={{ background:'var(--blue)', opacity:0.6, animation: `pulse-dot 1.2s ease ${i * 0.2}s infinite` }} />
                  ))}
                </div>
              </div>
            </div>
