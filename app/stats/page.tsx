...
export default function StatsPage() {
  ...
  const applyFilters = useCallback(() => {
    ...
    setFiltered(f)
  }, [allMovies, allEntries, search, language, genre, director, minRating, watchYears, rewatchFilter])
  ...
  const activeFilters = [search, language!=='All', genre!=='All', director!=='All', minRating>0, watchYears.length>0, rewatchFilter!=='All'].filter(Boolean).length

  const resetFilters = () => {
    setSearch(''); setLanguage('All'); setGenre('All')
    setDirector('All'); setMinRating(0); setWatchYears([]); setRewatchFilter('All')
  }
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select value={director} onChange={e => setDirector(e.target.value)} style={inputStyle}>
                    {directors.slice(0,120).map(d => <option key={d}>{d}</option>)}
                  </select>
                </div>
